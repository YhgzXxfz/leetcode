...
public:
	...
	int r = ring.size(), k = key.size();
        ...
        vector<vector<int>> cache(r, vector<int>(k));
        return dfs(r, key, 0, 0, dict, cache);
    }

private:
    int dfs(int r, string& key, int p1, int p2, vector<vector<int>>& dict, vector<vector<int>>& cache) {
        ...
        for (auto pos : dict[key[p2]-'a']) {
            int dist = abs(pos-p1);
            count = min(count, min(dist, r-dist) + dfs(r, key, pos, p2+1, dict, cache));
        }
        cache[p1][p2] = count+1;
        return cache[p1][p2];
    }
